...
class AvatarHistoryView(discord.ui.View):
    ...
    async def delete_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user.id == int(id_value):
            ...
            embed = discord.Embed(
                description=f"Successfully deleted avatar (ID: {idAV})", color=discord.Color.green())
...
class avhistory(commands.Cog):
        ...
        for av in avatars:
            try:
                ...
                m = await channel.send(
                    file=discord.File(io.BytesIO(fb), filename=fn),
                    content=text,
    ...
    async def avatarhistory(self, ctx, member: Union[discord.Member, int, str] = None):
        ...
        if len(data) == 0:
            embed = discord.Embed(description="No Avatars", color=discord.Color.green())
            await ctx.send(embed=embed)
            return
    ...
    async def collectavatars(self, ctx):
        ...
        embed = discord.Embed(description=f"{new} Avatars Collected", color=discord.Color.green())
        await ctx.send(embed=embed)
